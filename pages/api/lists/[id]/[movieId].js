    ...
    if (!session) {
      return response.status(401).send()
        ...
        try {
            ...
            const check = await database.collection('lists').findOne({_id: listId, user_id: user_id})
            if (check) {
              const result = await database.collection('lists_movies').deleteOne({ list_id: listId, movie_id: movieId})
