export default function moviesPipeline(options) {
  ...
  for (const [key, value] of Object.entries(options)) {
    if (value === "All" || value === "''") {
      ...
      matchPipeline['$match'][key] = { $elemMatch: { $regex: value, $options: 'i' } }
    }
      ...
      const formattedValue = fieldName === 'released' ? isMin ? new Date(`${value}-01-01T00:00:00.000Z`) : new Date(`${value}-12-31T00:00:00.000Z`) : parseFloat(value)
  ...
  pipeline.push({
    $facet: {
      movies: [
        ...
        { $limit: itemsPerPage },
        { $project: { _id: 1, poster: 1, title: 1, 'imdb.rating': 1, released: 1 } },
      ],
